...
from model.ETRT_CNN import ETRT_CNN

torch.manual_seed(123)

class ATD_ETRT_CNN(object):
    def __init__(self, args, df:pd.DataFrame):
        self.args = args
        self.device = self._acquire_device()
        self.ETRT_CNN = self._build_model(df).model
        self.model = self.ETRT_CNN.to(self.device)

    
    def _build_model(self, df):
        self.dim = self.args.dim
        self.history_len=self.args.history_len
        model = ETRT_CNN(history_len=self.history_len, predict_len=self.args.predict_len)
        self.model = model
        self.df = df
        return self
