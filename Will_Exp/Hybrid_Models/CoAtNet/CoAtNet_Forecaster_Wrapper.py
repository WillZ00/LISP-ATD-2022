...
import numpy as np
import pandas as pd
from CoAtNet.driver.atd_CoAtNet import atd_CoAtNet
from CoAtNet.utils.tools import dotdict

class CoAtNet_Forecaster():

    def __init__(self, args):
        self.args = args

    def fit(self, df:pd.DataFrame, past_covariates=None) -> "CoAtNet_Transformer_Forecaster":
        self.df=df
        self.training = pd.DataFrame(self._fit_processing(df), columns=df.columns, index=df.index)
        # add position encoding
        exp = atd_CoAtNet(self.args, self.training)
        exp.train()
        self.model = exp
        return self
