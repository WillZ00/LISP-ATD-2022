...
from CoAtNet.data.data_loader import atd_dataset, atd_Pred
from CoAtNet.model.CoAtNet import CoAtNet
...
class atd_CoAtNet(object):
    def __init__(self, args, df:pd.DataFrame):
        ...
        self.CoAtNet = self._build_model(df).model
        self.model = self.CoAtNet.to(self.device)

    
    def _build_model(self, df):
        self.dim = self.args.dim
        self.history_len=self.args.history_len
        model = CoAtNet(in_ch=260, image_size=20, out_chs = [260,520,1040,2080,5200])
        #model = CoAtNet(in_ch=260, image_size=20, out_chs = [260,460,800,1040,5200])
        #model = CoAtNet(in_ch=260, image_size=20, out_chs = [64,96,192,384,5200])
        self.model = model
        self.df = df
        return self
