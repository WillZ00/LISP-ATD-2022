...
from model.ST_CNN import ST_CNN

torch.manual_seed(123)

class ATD_ST_CNN(object):
    def __init__(self, args, df:pd.DataFrame):
        self.args = args
        self.device = self._acquire_device()
        self.CNN_Transformer_Net = self._build_model(df).model
        self.model = self.CNN_Transformer_Net.to(self.device)

    
    def _build_model(self, df):
        self.dim = self.args.dim
        self.history_len=self.args.history_len
        model = ST_CNN(history_len = self.history_len)
        self.model = model
        self.df = df
        return self
